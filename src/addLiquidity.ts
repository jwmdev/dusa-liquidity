import {
  ...
  } from "./lib/index.js"  //'@dusalabs/sdk'
  import { WalletClient } from '@massalabs/massa-web3'
  import { awaitFinalization, createClient, logEvents } from './utils.js'
  
  
  export const addLiquidity = async () => {
    console.log('------- adding liquidity ------------')
  
    const privateKey = "put here your private key"
    ...
    const typedValueUSDC = '2'
    const typedValueWMAS = '2'
    ...
    const numEachSideBins = 5
    const binRange = [lbPairData.activeId-numEachSideBins,lbPairData.activeId+numEachSideBins]
    const binParams = getCurveDistributionFromBinRange(lbPairData.activeId,binRange, [tokenAmountUSDC,tokenAmountWMAS])
   
  
    const params = pair.liquidityCallParameters({
      ...addLiquidityInput,
      deltaIds:binParams.deltaIds,
      distributionX: binParams.distributionX,
      distributionY: binParams.distributionY,
